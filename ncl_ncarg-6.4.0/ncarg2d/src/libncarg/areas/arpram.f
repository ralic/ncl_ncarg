...
C
            IF (.NOT.(IFRMXY.NE.0)) GO TO 10171
...
C                 original segment's end points for angle calculation.
                IF (.NOT.(IP2.LT.ILW.AND.IAM(IP2+3).GT.ILW)) GO TO 10180
...
10183           CONTINUE
C
                IF (.NOT.(IPU.EQ.1)) GO TO 10189
                  IF (.NOT.(ANG.GT.ANM)) GO TO 10190
                    IPR=IAM(IP2+3)
                    ANM=ANG
                    IPM=IPR
                    IPV=1
...
10179         CONTINUE
     ...
     +.IAM(IPP+1).OR.IAM(IAM(IP2+4)+2).NE.IAM(IPP+2)))) GO TO 10197
C                 Jira NCL_32: If test node is due to splitting a
C                 a line-segment to insert a hole-connector, use the
C                 original segment's end points for angle calculation.
                IF (.NOT.(IP2.LT.ILW.AND.IAM(IP2+4).GT.ILW)) GO TO 10198
...
10199           CONTINUE
                ...
                FYR=REAL(IYR)
C
                IF (.NOT.(IAU.EQ.1)) GO TO 10200
                  ANG=ARRAT2((FXQ-FXP)*(FYR-FYQ)-(FYQ-FYP)*(FXR-FXQ),
     +                       (FXQ-FXP)*(FXR-FXQ)+(FYQ-FYP)*(FYR-FYQ))
                GO TO 10201
10200           CONTINUE
                IF (.NOT.(IAU.EQ.2)) GO TO 10202
                  ANG=ARDAT2(DBLE(IXQ-IXP)*DBLE(IYR-IYQ)-
     +                       DBLE(IYQ-IYP)*DBLE(IXR-IXQ),
     +                       DBLE(IXQ-IXP)*DBLE(IXR-IXQ)+
     +                       DBLE(IYQ-IYP)*DBLE(IYR-IYQ))
...
10201           CONTINUE
C
                IF (.NOT.(IPU.EQ.1)) GO TO 10207
                  IF (.NOT.(ANG.GT.ANM)) GO TO 10208
                    IPR=IAM(IP2+4)
                    ANM=ANG
                    IPM=IP2
                    IPV=2
...
C
            IF (.NOT.(IPR.EQ.0)) GO TO 10215
              IPR=IPP
              IPV=3-IPV
              IF (.NOT.(IPU.EQ.1)) GO TO 10216
                ANM=+3.14159265358979
              GO TO 10217
10216         CONTINUE
                ANM=-3.14159265358979
...
C log an error.
C
            IF (.NOT.(IPV.EQ.1.AND.MOD(IAM(IPM),2).EQ.0)) GO TO 10218
              IAM(IPM)=IAM(IPM)+1
            GO TO 10219
10218       CONTINUE
            IF (.NOT.(IPV.EQ.2.AND.MOD(IAM(IPM)/2,2).EQ.0)) GO TO 10220
              IAM(IPM)=IAM(IPM)+2
            GO TO 10219
10220       CONTINUE
              IPT=IAM(5)-9
10221         CONTINUE
              IF (.NOT.(IPT.GT.ILW)) GO TO 10222
...
C
     ...
     +))  GO TO 10227
            ...
            IP1=IPH
10228       CONTINUE
            IF (.NOT.(IAM(IAM(IP1+5)+1).EQ.IAM(IPH+1))) GO TO 10229
              IP1=IAM(IP1+5)
            GO TO 10228
10229       CONTINUE
10230       CONTINUE
            IF (.NOT.(IAM(IP1+1).GE.IAM(IPH+1)-IAM(2))) GO TO 10231
              IF (.NOT.(ABS(IAM(IP1+7)).EQ.IGI.AND.IAM(IAM(IP1+4)+1).GT.
     +IAM(IP1+1).AND.IAM(IAM(IP1+4)+1).GE.IAM(IPH+1))) GO TO 10232
                IF (.NOT.(IAU.EQ.1)) GO TO 10233
                  YTM=REAL(IAM(IP1+2))+
     +            (XCI-REAL(IAM(IP1+1)))*
...
10234           CONTINUE
                IF (.NOT.(YTM.GT.YCI.AND.YTM.LT.YCO)) GO TO 10235
                  IOF=IP1
                  YCO=YTM
10235           CONTINUE
10232         CONTINUE
...
10238           CONTINUE
                IF (.NOT.(YTM.GT.YCI.AND.YTM.LT.YCO)) GO TO 10239
                  IOF=IAM(IP1+3)
                  YCO=YTM
10239           CONTINUE
10236         CONTINUE
            ...
            GO TO 10230
10231       CONTINUE
            IF (.NOT.(IOF.NE.0)) GO TO 10240
              IX0=IAM(IPH+1)
              IY0=IAM(IPH+2)
              IF (.NOT.(INT(YCO+.5).NE.IY0)) GO TO 10241
                IPI=18
                L10017=    6
                GO TO 10017
10242           CONTINUE
...
10241         CONTINUE
     ...
     +0.NE.IAM(IAM(IOF+4)+1).OR.IY0.NE.IAM(IAM(IOF+4)+2)))) GO TO 10244
                IPI=IOF
                L10017=    8
                GO TO 10017
10245           CONTINUE
...
C
        DO 10246 IPT=8,IAM(5)-9,10
          IAM(IPT)=4*(IAM(IPT)/4)
10246   CONTINUE
C
C If debugging is turned on, produce a plot.
C
        IF (.NOT.(IDB.NE.0)) GO TO 10247
...
10249   CONTINUE
     ...
     +AM(IPT)/2,2).NE.0.OR.IAM(IPT+9).GT.0))))) GO TO 10250
          IPT=IAM(IPT+5)
          IF (IPT.EQ.18) GO TO 10251
        GO TO 10249
10250   CONTINUE
C
C Pull out the group identifier for the segment.
...
C
        ...
        IAX=0
C
        IF (.NOT.(JRC.NE.0)) GO TO 10254
          ICN=0
          ICZ=0
          IOS=LAM-IAM(6)
          IF (.NOT.(IOS.GT.IAM(6)-1-IAM(5))) GO TO 10255
            CALL SETER ('ARPRAM - AREA-MAP ARRAY OVERFLOW',12,1)
            GO TO 10008
10255     CONTINUE
          DO 10257 ITM=IAM(6),LAM-1
            IAM(ITM-IOS)=0
10257     CONTINUE
10254   CONTINUE
C
...
C computing change in direction to a possible next point.
C
          IF (.NOT.(IFRMXY.NE.0)) GO TO 10261
C             Jira NCL_32: use original endpoint of segments that were
C             split for hole-connectors.
            IXP=IAM(IFRMXY+1)
            IYP=IAM(IFRMXY+2)
...
10275       CONTINUE
     ...
     +2+3)+2).NE.IAM(IPP+2)))) GO TO 10277
C                 Jira NCL_32: If test node is due to splitting a
C                 a line-segment to insert a hole-connector, use the
C                 original segment's end points for angle calculation.
                IF (.NOT.(IP2.LT.ILW.AND.IAM(IP2+3).GT.ILW)) GO TO 10278
                  IXR=IAM(IAM(IAM(IP2+3)+3)+1)
                  IYR=IAM(IAM(IAM(IP2+3)+3)+2)
                GO TO 10279
10278           CONTINUE
...
10279           CONTINUE
                ...
                FYR=REAL(IYR)
C
                IF (.NOT.(IAU.EQ.1)) GO TO 10280
                  ANG=ARRAT2((FXQ-FXP)*(FYR-FYQ)-(FYQ-FYP)*(FXR-FXQ),
     +                       (FXQ-FXP)*(FXR-FXQ)+(FYQ-FYP)*(FYR-FYQ))
                GO TO 10281
10280           CONTINUE
                IF (.NOT.(IAU.EQ.2)) GO TO 10282
                  ANG=ARDAT2(DBLE(IXQ-IXP)*DBLE(IYR-IYQ)-
     +                       DBLE(IYQ-IYP)*DBLE(IXR-IXQ),
     +                       DBLE(IXQ-IXP)*DBLE(IXR-IXQ)+
     +                       DBLE(IYQ-IYP)*DBLE(IYR-IYQ))
...
10281           CONTINUE
C
                IF (.NOT.(IPU.EQ.1)) GO TO 10287
                  IF (.NOT.(ANG.GT.ANM)) GO TO 10288
...
C                 a line-segment to insert a hole-connector, use the
C                 original segment's end points for angle calculation.
                IF (.NOT.(IP2.LT.ILW.AND.IAM(IP2+4).GT.ILW)) GO TO 10300
                  IXR=IAM(IAM(IAM(IP2+4)+4)+1)
                  IYR=IAM(IAM(IAM(IP2+4)+4)+2)
                GO TO 10301
10300           CONTINUE
...
10301           CONTINUE
                ...
                FYR=REAL(IYR)
C
                IF (.NOT.(IAU.EQ.1)) GO TO 10302
                  ANG=ARRAT2((FXQ-FXP)*(FYR-FYQ)-(FYQ-FYP)*(FXR-FXQ),
     +                       (FXQ-FXP)*(FXR-FXQ)+(FYQ-FYP)*(FYR-FYQ))
                GO TO 10303
10302           CONTINUE
                IF (.NOT.(IAU.EQ.2)) GO TO 10304
                  ANG=ARDAT2(DBLE(IXQ-IXP)*DBLE(IYR-IYQ)-
     +                       DBLE(IYQ-IYP)*DBLE(IXR-IXQ),
     +                       DBLE(IXQ-IXP)*DBLE(IXR-IXQ)+
     +                       DBLE(IYQ-IYP)*DBLE(IYR-IYQ))
...
10303           CONTINUE
C
                IF (.NOT.(IPU.EQ.1)) GO TO 10309
                  IF (.NOT.(ANG.GT.ANM)) GO TO 10310
C                     Jira NCL_32: special-case code for Tomas example:
C                     Do not follow hole-connector segments that double
C                     back in the direction we just came from.
                    IF (.NOT.(ISHOLEC(IP2).AND.ANG.GE.3.14159265358979))
...
C
          IF (.NOT.(IPV.EQ.1.AND.MOD(IAM(IPM),2).EQ.0)) GO TO 10322
            IAM(IPM)=IAM(IPM)+1
            IAQ=IPM+8
          GO TO 10323
10322     CONTINUE
          IF (.NOT.(IPV.EQ.2.AND.MOD(IAM(IPM)/2,2).EQ.0)) GO TO 10324
            IAM(IPM)=IAM(IPM)+2
            IAQ=IPM+9
          GO TO 10323
10324     CONTINUE
...
10323     CONTINUE
C
          IF (.NOT.(IAM(IAQ).LE.0.OR.IAM(IAQ).GE.IAM(6))) GO TO 10326
C
            IF (.NOT.(JRC.EQ.0)) GO TO 10327
C
              IF (.NOT.(IAM(IAQ).LT.0)) GO TO 10328
                IAI=-1
                IAX=LAM*4
              GO TO 10329
10328         CONTINUE
...
10327       CONTINUE
C
              IF (.NOT.(IAM(IAQ).LT.0)) GO TO 10332
                ICN=ICN+1
              GO TO 10333
10332         CONTINUE
              IF (.NOT.(IAM(IAQ).EQ.0)) GO TO 10334
                ICZ=ICZ+1
              GO TO 10333
10334         CONTINUE
              IF (.NOT.(IAM(IAQ).GE.IAM(6))) GO TO 10335
...
C
     ...
     +IAM(IPR+1).EQ.IAM(IPT+1).AND.IAM(IPR+2).EQ.IAM(IPT+2)) GO TO 10336
C
        GO TO 10260
10336   CONTINUE
C
...
C
        IF (.NOT.(JRC.LT.0.AND.ICN.GT.0)) GO TO 10337
          IAI=-1
        GO TO 10338
10337   CONTINUE
        IF (.NOT.(JRC.NE.0)) GO TO 10339
          IF (.NOT.(ICN.GT.0)) GO TO 10340
            IAI=-1
            IAX=ICN
10340     CONTINUE
          IF (.NOT.(ICZ.GT.IAX.AND.ABS(JRC).EQ.2)) GO TO 10341
            IAI=0
            IAX=ICZ
10341     CONTINUE
          DO 10342 ITM=IAM(6),LAM-1
            IF (.NOT.(IAM(ITM-IOS).GE.IAX)) GO TO 10343
              IAI=ITM
              IAX=IAM(ITM-IOS)
10343       CONTINUE
10342     CONTINUE
...
C
        IF (.NOT.(IAI.EQ.0.AND.KF3.NE.0)) GO TO 10346
          DO 10347 IPT=8,IAM(5)-9,10
            IAM(IPT)=4*(IAM(IPT)/4)
10347     CONTINUE
          KF3=0
          GO TO 104
...
10017 CONTINUE
        IPN=IAM(5)+1
        IF (.NOT.(IAM(5)+10.GE.IAM(6))) GO TO 10352
...
10352   CONTINUE
        ...
        IAM(IPI+4)=IPN
10354   CONTINUE
          IF (.NOT.(IAM(IPN+1).LT.IAM(IPX+1))) GO TO 10355
            IPX=IAM(IPX+6)
          GO TO 10356
10355     CONTINUE
          IF (.NOT.(IAM(IPN+1).GT.IAM(IAM(IPX+5)+1))) GO TO 10357
            IPX=IAM(IPX+5)
          GO TO 10356
10357     CONTINUE
10358       CONTINUE
...
10359         CONTINUE
     ...
     +T.IAM(IAM(IPX+5)+2))) GO TO 10361
                IPX=IAM(IPX+5)
              GO TO 10360
10361         CONTINUE
10362           CONTINUE
                IF (.NOT.(IAM(IAM(IPX+5)+1).EQ.IAM(IPN+1).AND.IAM(IAM(IP
     +X+5)+2).EQ.IAM(IPN+2))) GO TO 10363
                  IPX=IAM(IPX+5)
                GO TO 10362
10363           CONTINUE
                GO TO 10364
...
10365   CONTINUE
        ...
        IAM(IPN+8)=IAM(IPI+8)
        IAM(IPN+9)=IAM(IPI+9)
      GO TO (10016,10032,10083,10085,10097,10242,10243,10245) , L10017
C
